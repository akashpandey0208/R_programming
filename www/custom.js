...
// =============================================================================
// DATA MANAGEMENT SUBMENU — hover card with bridge-delay
// =============================================================================
(function() {
  var dmTimer = null;

  // Show card on hover over the wrapper (button + card)
  $(document).on('mouseenter', '.menu-item-wrapper.has-submenu', function() {
    clearTimeout(dmTimer);
    $(this).find('.dm-submenu-card').stop(true, true).fadeIn(130);
  });

  // Start hide timer when leaving the wrapper
  $(document).on('mouseleave', '.menu-item-wrapper.has-submenu', function() {
    var $card = $(this).find('.dm-submenu-card');
    dmTimer = setTimeout(function() { $card.fadeOut(130); }, 200);
  });

  // Keep card open when mouse enters the card itself
  $(document).on('mouseenter', '.dm-submenu-card', function() {
    clearTimeout(dmTimer);
  });

  // Hide when mouse leaves the card
  $(document).on('mouseleave', '.dm-submenu-card', function() {
    var $card = $(this);
    dmTimer = setTimeout(function() { $card.fadeOut(130); }, 200);
  });

  // On load, highlight the default active sub-item if a value is already set
  $(document).on('shiny:sessioninitialized', function() {
    var sub = Shiny.shinyapp.$inputValues && Shiny.shinyapp.$inputValues['dm_sub_menu'];
    if (sub) {
      var target = sub === 'data_upload' ? '#dm-sub-data-upload' : '#dm-sub-data-operations';
      $(target).addClass('dm-sub-active');
    }
  });
})();

// =============================================================================
// FOLDER TREE — expand/collapse toggles
// =============================================================================
$(document).on('click', '.du-toggle', function(e) {
  e.stopPropagation();
  var $node     = $(this).closest('.du-tree-node');
  var $children = $node.siblings('.du-tree-children').first();
  if (!$children.length) {
    $children = $node.parent().find('> .du-tree-children').first();
  }
  $(this).toggleClass('open');
  $children.slideToggle(120);
});

// Upload drop-zone drag styling
$(document).on('dragover dragenter', '.du-upload-zone', function() {
  $(this).addClass('dragover');
}).on('dragleave drop', '.du-upload-zone', function() {
...
document.addEventListener('dragstart', function(e){
  if(e.target.classList.contains('drag-var')){
    e.dataTransfer.setData('text', e.target.dataset.var);
  }
});
...
document.addEventListener('drop', function(e){
  ...
  if(e.target.classList.contains('drop-container')){
    ...
    let varName = e.dataTransfer.getData('text');

    let id = e.target.id;

    Shiny.setInputValue(id + "_dropped_var", varName, {priority:'event'});
