...
login_controller <- function(input, output, session) {
  ...
  observeEvent(input$stored_access_token, {
    ...
    if (!is.null(access) && nzchar(access)) {
      res <- GET(
        paste0(Sys.getenv("API_BASE_URL"), "/secure_data"),
      ...
      if (status_code(res) == 200) {
        ...
        name_res <- GET(
        paste0(Sys.getenv("API_BASE_URL"), "/user_fullname"),
        add_headers(Authorization = paste("Bearer", access))
      )
    ...
    if (!is.null(refresh) && nzchar(refresh)) {
      refresh_res <- POST(
        paste0(Sys.getenv("API_BASE_URL"), "/refresh"),
  ...
  observeEvent(input$login_btn, {
    ...
    res <- POST(
      paste0(Sys.getenv("API_BASE_URL"), "/login"),
    ...
    if (status_code(res) == 200) {
      ...
      session$userData$access_token(tokens$access_token)
      session$userData$refresh_token(tokens$refresh_token)
      
      # Fetch full name from API using access token
      name_res <- GET(
        paste0(Sys.getenv("API_BASE_URL"), "/user_fullname"),
      ...
      if (status_code(name_res) == 200) {
        ...
        session$userData$full_name(NULL)
      }
  ...
  observeEvent(input$logout_btn, {
    ...
    POST(
      paste0(Sys.getenv("API_BASE_URL"), "/logout"),
      body = list(email = input$email_value),
      encode = "json"
    )
